...
async function upgradeTomcat(tomcatVersion, tomcatUrl, javaVersion) {
  ...
  try {
    ...
    // ✅ Check if the 'tomcat' group exists before creating it
    try {
      await runCommand(`getent group tomcat`);
      console.log("✅ 'tomcat' group already exists. Skipping group creation.");
    } catch (error) {
      console.log("🔄 Creating 'tomcat' group...");
      await runCommand(`sudo groupadd tomcat`);
    }

    // ✅ Add the 'tomcat' user if it doesn't exist
    try {
      await runCommand(`id -u tomcat`);
      console.log("✅ 'tomcat' user already exists. Skipping user creation.");
    } catch (error) {
      console.log("🔄 Creating 'tomcat' user...");
     // await runCommand(`sudo adduser --system --no-create-home --group tomcat`);
	  await runCommand(`sudo useradd --system --no-create-home --gid tomcat tomcat`);
    }
